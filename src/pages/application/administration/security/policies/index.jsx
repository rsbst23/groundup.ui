...
const PoliciesList = () => {
  ...
  const tableProps = useTableConfig({
    tableOptions: {
      fetchAction: fetchPolicies,
      removeAction: removePolicy,
      exportAction: exportPoliciesData,
      dataSelector: (state) => ({
        items: state.policies.policies,
        loading: state.policies.loading,
        error: state.policies.error,
        totalRecords: state.policies.totalRecords,
      }),
      defaultSort: "name",
      contextName: "Policies",
    },
    columnDefs,
    translationPrefix: "",
  });
